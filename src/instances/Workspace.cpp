...
void Workspace::Bind(lua_State* L) {
    ...
    //Gravity property
    LuaClassBinder::AddProperty("Workspace", "Gravity", [](lua_State* L, Instance* inst) -> int {
    ...
    LuaClassBinder::AddProperty("Workspace", "CurrentCamera", [](lua_State* L, Instance* inst) -> int {
            auto* ws = static_cast<Workspace*>(inst);
            if (ws->CurrentCamera) {
